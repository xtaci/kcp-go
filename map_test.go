...
func BenchmarkConcurrentMap_Write(b *testing.B) {
	...
	for i := 0; i < concurrency; i++ {
		wait.Add(1)
		go func(m *ConcurrentMap, start, end int, data []string, wait *sync.WaitGroup) {
			for i := start; i < end; i++ {
				m.Put(data[i], i)
			}
			wait.Done()
		}(newMap, i*workLoad, (i+1)*workLoad, data, wait)
	}
	wait.Wait()
}
...
func BenchmarkSyncMap_Write(b *testing.B) {
	...
	for i := 0; i < concurrency; i++ {
		wait.Add(1)
		go func(m *sync.Map, start, end int, data []string, wait *sync.WaitGroup) {
			for i := start; i < end; i++ {
				m.Store(data[i], i)
			}
			wait.Done()
		}(oldMap, i*workLoad, (i+1)*workLoad, data, wait)
	}
	wait.Wait()
}
...
func BenchmarkConcurrentMap_ReadWrite(b *testing.B) {
	...
	for i := 0; i < concurrency; i++ {
		wait.Add(1)
		r := rand.Int()
		go func(w *sync.WaitGroup, seed int, m *ConcurrentMap) {
			for j := 0; j < b.N; j++ {
				l := rand.Intn(dataLen)
				if l%2 == 0 {
					m.Get(strconv.Itoa(l))
				} else {
					m.Put(strconv.Itoa(l), l+seed)
				}
			}
			w.Done()
		}(wait, r, m)
...
func BenchmarkSyncMap_ReadWrite(b *testing.B) {
	...
	for i := 0; i < concurrency; i++ {
		wait.Add(1)
		r := rand.Int()
		go func(w *sync.WaitGroup, seed int, m *sync.Map) {
			for j := 0; j < b.N; j++ {
				l := rand.Intn(dataLen)
				if l%2 == 0 {
					m.Load(strconv.Itoa(l))
				} else {
					m.Store(strconv.Itoa(l), l+seed)
				}
			}
			w.Done()
		}(wait, r, m)
