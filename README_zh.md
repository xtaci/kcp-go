...
- 基于 Reed-Solomon 的编码/解码逻辑直接嵌入 `postProcess` 与 `packetInput` 流水线，无需额外 goroutine 或锁竞争即可生成、消费冗余分片。
- 每条会话都可以调节数据/冗余比例，在 ~20–30% 额外带宽开销与长途、丢包链路的尾部时延之间自由取舍。
- 冗余分片复用缓冲池切片产出，避免重复分配与清零，即便在多 Gbps 传输下也能维持稳定的 GC 压力。
- 解码过程倾向单次恢复：一旦收齐足量分片，就立刻重建原始报文并投递给 `KCP.Input`，最大限度减少乱序与重传风暴。
- 与加密层叠加时，FEC 头部同样受到保护，流量整形设备难以推断恢复节奏或借机压制吞吐。
